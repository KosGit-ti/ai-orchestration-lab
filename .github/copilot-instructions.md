...
10. Copilot コードレビュー対応（初回レビューのみ、最大3回の指摘対応）
    ...
    - Must/Should 指摘があれば implementer に修正を委譲する
    - 各レビューコメントに対応結果を `gh api .../comments/{id}/replies` で返信する
    - **コミット直前ゲート**（必須）: コミット直前に CI（検証モード: `--check` フラグ付き）+ get_errors を実行し、通過を確認する。CI 実行後にコードファイルを変更した場合は CI を再実行すること
    - **コミット後クリーン検証**（必須）: コミット後に `git status --porcelain` で出力が空であることを確認する。出力が空でない場合はコミット直前ゲートに戻る（直接追加コミットしない）
    - **コメント安定化フェーズ**: レビュー検出後、コメント数が安定するまで追加待機する（15秒×最大8回）
...
8. **Copilot レビュー対応**: 自動実行モード Step 10 に準拠（**最大20分（60秒×最大20回）の待機を含む**）
